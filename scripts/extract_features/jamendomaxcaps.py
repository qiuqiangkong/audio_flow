...
def extract_audio_features(args) -> None:
    ...
    # asdf
    parquet_path = root / f"data/train-{subset}-of-02272.parquet"
    ...
    for n in range(n_data):
        ...
        audio = load_audio_from_bytes(audio_bytes, encoder.sr)
        # sf.write(file="_zz.wav", data=audio.T, samplerate=encoder.sr)
        # asdf
...
def load_audio_from_bytes(audio_bytes: np.ndarray, sr: float) -> np.ndarray:
    ...
    return librosa.resample(y=audio, orig_sr=orig_sr, target_sr=sr).astype(np.float32)
