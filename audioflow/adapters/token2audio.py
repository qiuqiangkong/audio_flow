...
from audioflow.adapters.wrappers import T5Wrapper, ClapTextWrapper, TokenWrapper


class Token2AudioAdapter(nn.Module): 
    def __init__(self, n_quantizers: int, codebook_size: int, dim: int, **kwargs):
        super().__init__()

        self.t5 = T5Wrapper(dim)
        self.clap_text = ClapTextWrapper(dim)
        self.feature_encoder = TokenWrapper(n_quantizers, codebook_size, dim)
