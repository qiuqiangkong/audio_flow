...
class Any2AudioDataset:
    ...
    def __getitem__(self, meta: dict) -> dict:
        ...
        in_path = meta["input"]["feature"]["path"]
        in_fps = meta["input"]["feature"]["fps"]

        # Target
        tgt_path = meta["target"]["latent"]["path"]
        tgt_fps = meta["target"]["latent"]["fps"]
        dur = min(meta["input"]["feature"]["duration"], meta["target"]["latent"]["duration"])
